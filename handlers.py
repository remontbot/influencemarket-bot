...
# Тематики контента для блогеров
...
async def select_role(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "blogger":
        await query.edit_message_text(
            ...
            "чтобы рекламодатели могли быстро находить вас.\n\n"
            "🔹 Вам не нужно рекламировать себя или платить за продвижение.\n"
            "🔹 Вы просто берёте кампании, создаёте контент и растёте в рейтинге.\n\n"
            "Чем выше качество контента, общение и соблюдение сроков —\n"
            "тем лучше и чаще вам будут приходить кампании.\n\n"
            "Сейчас самое время честно описать свой опыт и тематику:\n"
            "кому-то важен максимальный охват,\n"
            "кому-то — более доступная цена.\n"
            "Это помогает системе подбирать подходящие кампании именно вам.\n\n"
        ...
        await query.edit_message_text(
            "Сейчас мы создадим для вас профиль рекламодателя.\n\n"
            "Он нужен, чтобы блогеры видели, с кем общаются, "
            "а вы могли сохранять историю кампаний и удобно выбирать исполнителей.\n\n"
            "✏️ Введите ваше имя.",
            parse_mode="HTML",
        )
...
async def register_blogger_cities_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "add_more_cities":
        ...
        await query.edit_message_text(
            ...
            "📱 <b>Шаг 4/7:</b> Выберите основную тематику контент:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(keyboard),
        )
...
async def register_blogger_main_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка выбора основной категории контент"""
    query = update.callback_query
    await query.answer()
...
async def register_blogger_ask_more_categories(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "yes":
        ...
        await query.edit_message_text(
            ...
            "можно получить сложные кампании, низкие оценки и потерять рейтинг.\n\n"
